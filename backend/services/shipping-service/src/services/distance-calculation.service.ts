...
export class DistanceCalculationService {
  ...
  private readonly postalCodeCoordinates: Record<
    string,
  ...
  async calculateDistance(
    ...
    // 1) Cache
    const cached = await this.cacheService.getDistance(
      fromPostalCode,
      toPostalCode,
    );
    if (cached !== null) {
  ...
  private parseApiResponse(
    ...
    if (data?.distance)
      return typeof data.distance === 'number'
        ? data.distance
        : parseFloat(data.distance);
    throw new Error(
      `Could not parse API response for ${fromPostalCode} -> ${toPostalCode}`,
    );
  }

  private async calculateManually(
    fromPostalCode: string,
    toPostalCode: string,
  ): Promise<number> {
