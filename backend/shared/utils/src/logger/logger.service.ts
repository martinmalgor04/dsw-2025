...
export class LoggerService extends Logger {
   ...
   */
  logWithContext(message: string, context: LogContext, logLevel: 'log' | 'error' | 'warn' | 'debug' = 'log') {
    const contextString = Object.keys(context).length > 0 ? JSON.stringify(context) : '';
    const fullMessage = contextString ? `${message} - Context: ${contextString}` : message;
  ...
  startOperation(operation: string, context: LogContext = {}) {
    this.logWithContext(`Starting operation: ${operation}`, {
      ...context,
      operation,
  ...
  endOperation(operation: string, context: LogContext = {}, duration?: number) {
    this.logWithContext(`Completed operation: ${operation}`, {
      ...context,
      operation,
  ...
  apiCall(method: string, url: string, context: LogContext = {}) {
    this.logWithContext(`API Call: ${method} ${url}`, {
      ...context,
      method,
      url,
      timestamp: new Date().toISOString(),
  ...
  apiResponse(method: string, url: string, statusCode: number, duration: number, context: LogContext = {}) {
    this.logWithContext(`API Response: ${method} ${url} - ${statusCode}`, {
      ...context,
      method,
      url,
      statusCode,
